...
export function HeatmapCalendar({
  ...
  const tooltipNode = (cell: HeatmapCell) => {
    ...
    const unit = cell.value === 1 ? "review" : "reviews"
    return (
    ...
    <Card className={cn(className, "w-full")}>
      ...
      <CardContent>
        <TooltipProvider delayDuration={80}>
          <div
            className={cn(
              "flex w-full gap-4 overflow-x-auto overflow-y-auto justify-center items-center",
              placement === "bottom" && "flex-col",
            )}
          >
            {/* Labeled calendar area */}
            <div className={cn("w-full", axisCfg.className)}>
              {" "}
              {/* Month labels row */}
              {showAxis && showMonths ? (
                <div
